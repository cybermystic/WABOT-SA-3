...
${chalk.blue.italic('ℹ️ Connecting to WhatsApp... Please wait.')}`);
    ...
    conn.on('open', async () => {
        ...
        console.log(
            chalk.green.bold('WABOT-SA bot working correctly 😍')
        );
    });
                        ...
                        try {
                            ...
                            if (config.LANG == 'TR' || config.LANG == 'AZ') {
                                ...
                                await conn.sendMessage(conn.user.jid, '*-- ERROR REPORT [WABOT-SA] --*' + 
                                    ...
                                    '\n_Joij our WhatsApp group and ask what is ERROR https://chat.whatsapp.com/GS6WZFCyrl60pszygT9wPS' +
                                    '\n_This message should have gone to your number (saved messages)._\n\n' +
                                    '*Error:* ```' + error + '```\n\n'
                                    , MessageType.text, {detectLinks: false}
                                );
                                if (error.message.includes('URL')) {
                                    return await conn.sendMessage(conn.user.jid, '*😕 ERROR ANALYSIS [WABOT-SA] 😕*' + 
                                        '\n========== ```Error Resolved!``` ==========' +
